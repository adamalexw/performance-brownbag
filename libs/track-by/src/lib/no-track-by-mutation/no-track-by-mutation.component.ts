...
@Component({
  selector: 'performance-brownbag-no-track-by-mutation',
  templateUrl: './no-track-by-mutation.component.html',
  styleUrls: ['./no-track-by-mutation.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class NoTrackByMutationComponent {
  teams: F1Team[];
  ...
  onAddTeam(team: string) {
    this.teams.push(({ name: team, logo: `https://www.formula1.com/content/dam/fom-website/teams/2022/${team.toLowerCase().replace(/ /g, '-')}-logo.png.transform/2col/image.png` }));
  }
